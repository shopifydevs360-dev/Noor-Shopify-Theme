...
function initVariantPriceUpdate() {
  ...
  if (!form || !priceItems.length || !window.product || !variantInput) return;
  ...
  function togglePrice(variantId) {
    priceItems.forEach(item => {
      item.classList.toggle(
        'show-price',
        item.dataset.variantId === String(variantId)
      );
      item.classList.toggle(
        'hide-price',
        item.dataset.variantId !== String(variantId)
...
function initMainProductCart() {
  ...
  form.addEventListener('submit', e => {
    // 🔑 FIX: allow normal submit for redirect
...
/* =================================
   VARIANT BUTTON STATE LOGIC (RESTORED)
================================= */
